...
void CAABonderDlg::Rs232Init()
{
	...
	//1호기 M1_TANGERINE_5M 모델 이물광원 1채널 
...
void CAABonderDlg::SetInterface_Label()
{
	...
	else if (LGIT_MODEL_INDEX == M1_TANGERINE_5M)
	{
		temp.Format(_T("M1_TANGERINE_5M"));
		ModelSelect = true;
	}
#else
	...
	for (j = 0; j < iCnt_Focus; j++)		//for (j = 0; j < 1; j++)//
    {
        ...
        for (i = 0; i < sfrMax; i++)
        {
            ...
            if (pROIResult)
            {
                ...
                {
					Task.SFR._64_fSfrN4[i] = sfrValue * 1.37;   //250403 TENGERINE_M5
                }
                //
...
bool CAABonderDlg::func_MTF(BYTE* ChartRawImage, bool bAutoMode, int dindex)
{
	...
	for (j = 0; j < iCnt_MtfFocus; j++)
	{
		...
		for (i = 0; i < sfrMax; i++)
		{
			...
			if (pROIResult)
			{
				...
				model.m_LogSfr[i] = sfrValue * 1.37;   //250403 TENGERINE_M5
				if (bMtfMaxTest)
				{
					if (Task.PCBTask == 65000 || Task.PCBTask == 60200)
					{
	...
	if (bMtfMaxTest == false)
	{
		for (i = 0; i < model.mGlobalSmallChartCount; i++)
		{
			...
			model.m_LogSfr[i] = fMax_MTF_N4[i] * 1.37;   //250403 TENGERINE_M5
...
void CAABonderDlg::OnClickedLabelTitle()
{
	...
	OcLight_Dms50v52.DMS_50V5_2_OnOff(1, true);
	//OcLight_Dms50v52.DMS_50V5_2_Value(1, 255);
