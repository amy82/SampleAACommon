...
void CLensDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_INSP_MOTOR_COM_ALIGN, m_btnInspXComAlignPos);
...
//	ON_WM_HSCROLL()
	...
	ON_BN_CLICKED(IDC_BUTTON_INSP_MOTOR_COM_ALIGN, &CLensDlg::OnBnClickedButtonInspMotorComAlign)
END_MESSAGE_MAP()
...
void CLensDlg::setInterface()
{
	...
	GetDlgItem(IDC_BUTTON_INSP_MOTOR_WAIT)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BUTTON_INSP_MOTOR_ALIGN)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BUTTON_INSP_MOTOR_DEFECT_INSP)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BUTTON_INSP_MOTOR_DARK_INSP)->ShowWindow(SW_HIDE);
#endif
}
...
void CLensDlg::OnBnClickedButtonAlignTest()
{
	...
	if (iRtn == 0)
	{
		...
		else
		{
			...
			axis[2] = Motor_PCB_TH;
#if (__MACHINE_MODEL == MACHINE_1ST)
			pos[0] = motor.GetEncoderPos(Motor_PCB_X) - offsetX;
			pos[1] = motor.GetEncoderPos(Motor_PCB_Y) - offsetY;
...
void CLensDlg::changeColorMotorPosBtn()
{
	...
	else if (checkCurInspXMotorPos(CompleteAlign_Pos))					iIndex = 4;

	m_btnInspXWaitPos.m_iStateBtn = 0;
	m_btnInspXAlignPos.m_iStateBtn = 0;
	m_btnInspXComAlignPos.m_iStateBtn = 0;
	...
	else if (iIndex == 4)		m_btnInspXDarkPos.m_iStateBtn = 3;
...
void CLensDlg::OnBnClickedButtonInspMotorComAlign()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
