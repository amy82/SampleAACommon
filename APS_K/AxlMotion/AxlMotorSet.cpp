...
int CAxlMotorSet::Axl_Axisconfig() 
{
	...
	for(int i=0; i<MAX_MOTOR_NO; i++)
	{
		...
		if (i == Motor_PCB_X || i == Motor_PCB_Y || i == Motor_PCB_Z || i == Motor_PCB_Xt || i == Motor_PCB_Yt || i == Motor_Lens_X || i == Motor_Lens_Y || i == Motor_Lens_Z || i == Motor_Lens_Yt ||i == Motor_Lens_Xt || i == Motor_Lens_TH
			|| i == Motor_Insp_X || i == Motor_LensAlign_X || i == Motor_LensAlign_Y || i == Motor_LensAlign_Z || i == Motor_LensAlign_Xt || i == Motor_LensLaser_Yt || i == Motor_LensLaser_Xt)
#endif
		{
		...
		if( MOTOR_TYPE[i] == STEPING)				// Step 모터 드라이브는 출력 방식, 리미트 설정 틀림.
		{
			...
			dwReturn = AxmSignalSetStop(i, EMERGENCY_STOP, LOW); //HIGH);
#endif
			if ( dwReturn != AXT_RT_SUCCESS )
			{
		...
		else
		{
			...
			if (i == Motor_PCB_X || i == Motor_PCB_Z || i == Motor_Lens_Z || i == Motor_LensAlign_Z || i == Motor_LensAlign_Y)
			{
				dwReturn = AxmSignalSetLimit(i, EMERGENCY_STOP, LOW, LOW);//low 기존
			}
			else
		...
		if (i == Motor_PCB_Z || i == Motor_Lens_Z || i == Motor_LensAlign_Z)
		{
			dwReturn = AxmSignalSetServoAlarm(i, LOW);
		}
		else
		{
			dwReturn = AxmSignalSetServoAlarm(i, HIGH);
...
//! 원점 속도 조절 할것.
	...
	case Motor_PCB_X:			//! PCB 공급 X축
		dVelFirst = 30000.;		dVelSecond = 20000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;
	...
	case Motor_Lens_X:			//! LENS 공급 X축
		dVelFirst = 30000.;		dVelSecond = 10000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;
	...
	case Motor_Insp_X:			//! PCB 공급 X축
		dVelFirst = 30000.;		dVelSecond = 20000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_Lens_TH:			//! PCB 공급 THETA축
		dVelFirst = 5000.;		dVelSecond = 4000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_X:			//! LENS 공급 X축
		dVelFirst = 30000.;		dVelSecond = 10000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_Y:			//! LENS 공급 Z축
		dVelFirst = 10000.;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_Z:			//! LENS 공급 Z축
		dVelFirst = 10000.;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_TH:			//! LENS 공급 TY축
		dVelFirst = 5000;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_Yt:			//! LENS 공급 TY축
		dVelFirst = 5000;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensAlign_Xt:			//! LENS 공급 TY축
		dVelFirst = 5000;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensLaser_Yt:			//! LENS 공급 TY축
		dVelFirst = 5000;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;

	case Motor_LensLaser_Xt:			//! LENS 공급 TY축
		dVelFirst = 5000;		dVelSecond = 3000.;		dVelThird = 500;		dvelLast = 50.;		dAccFirst = 0.1;	dAccSecond = 0.1;
		break;
#endif
...
bool CAxlMotorSet::HomeProcessAll_2st()
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	CString sLog="";
	CTimeChecker timeChk;
	...
	for (int i = Motor_PCB_X; i < MAX_MOTOR_NO; i++)//(int i = Motor_PCB_X; i <= Motor_Lens_Z; i++)//for (int i= Motor_Lens_X ; i<= Motor_PCB_TH ; i++) 
	{
		m_bOrgFlag[i] = false;
	...
	while(true)
	{
		...
		for(int i =0;i<MAX_MOTOR_NO;i++)
		{
			if(!GetAmpEnable(i))
			{
				if(!(i == Motor_Lens_Xt  && iStep[Motor_Lens_Xt] == 600) &&
					...
					!(i == Motor_Lens_TH  && iStep[Motor_Lens_TH] == 600) &&
					!(i == Motor_PCB_TH  && iStep[Motor_PCB_TH] == 600) &&
					!(i == Motor_LensAlign_Z  && iStep[Motor_LensAlign_Z] == 600) &&
					!(i == Motor_LensAlign_Yt  && iStep[Motor_LensAlign_Yt] == 600) &&
					!(i == Motor_LensAlign_Xt  && iStep[Motor_LensAlign_Xt] == 600) &&
					!(i == Motor_LensLaser_Yt  && iStep[Motor_LensLaser_Yt] == 600) &&
					!(i == Motor_LensLaser_Xt  && iStep[Motor_LensLaser_Xt] == 600) )		//!(i == Motor_LensAlign_TH  && iStep[Motor_LensAlign_TH] == 600) &&
				{
					sLangChange.LoadStringA(IDS_STRING1279);	//서보 Off 발생.
					errMsg2(Task.AutoFlag, sLangChange);
					return false;
		...
		case 220:
			...
			AxmMoveVel(Motor_PCB_Z, -dVel, dAcc, dAcc);	
			checkTime = myTimer(true);// 음수 이동
			MainStep = 250;
			break;
			//Laser 실린더 상승 END
		...
		case 250:
			...
			{
				dAcc = sysData.fMotorAccTime[Motor_Insp_X];
				dVel = 20 * sysData.fMotorResol[Motor_Insp_X];
				AxmMoveVel(Motor_Insp_X, -dVel, dAcc, dAcc);
				checkTime = myTimer(true);
				MainStep = 300;
			}
			else if ((curTime - checkTime) > MOTOR_MOVE_TIME)
			{
				//sLangChange.LoadStringA(IDS_STRING337);
				errMsg2(Task.AutoFlag, "[원점] PCB Z축 (-)이동 실패");
				MainStep = -250;
			}
			break;
		...
		case 405://! PCB Z축 (-) Limit 위치 이동
			if(GetNegaSensor(Motor_PCB_Z) && IsStopAxis(Motor_PCB_Z))
			{
				dAcc = sysData.fMotorAccTime[Motor_LensAlign_Z];
				dVel = 5 * sysData.fMotorResol[Motor_LensAlign_Z];
				AxmMoveVel(Motor_LensAlign_Z, -dVel, dAcc, dAcc);		// 음수 이동
				checkTime = myTimer(true);
				MainStep = 410;
			}
			else if((curTime - checkTime) > MOTOR_MOVE_TIME)
			...
			break;
		case 410://! Lens Align Z축 (+) Limit 위치 이동
			if (GetNegaSensor(Motor_LensAlign_Z) && IsStopAxis(Motor_LensAlign_Z))
			{
			...
			{
				//sLangChange.LoadStringA(IDS_STRING338);
				sLangChange.Format("LensAlign Z-Axis Move Fail");
				errMsg2(Task.AutoFlag, sLangChange);
				MainStep = -410;
			}
			break;
		case 411://! Lens Y축 (+) Limit 위치 이동
			...
			break;
		case 420:
			dAcc = sysData.fMotorAccTime[Motor_LensAlign_X];
			dVel = 3 * sysData.fMotorResol[Motor_LensAlign_X];
			AxmMoveVel(Motor_LensAlign_X, dVel, dAcc, dAcc);
			//AxmMoveSignalSearch(Motor_PCB_Xt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			checkTime = myTimer(true);
			MainStep = 421;
			break;
		case 421:
			dAcc = sysData.fMotorAccTime[Motor_LensAlign_Y];
			dVel = 3 * sysData.fMotorResol[Motor_LensAlign_Y];
			AxmMoveVel(Motor_LensAlign_Y, -dVel, dAcc, dAcc);
			//AxmMoveSignalSearch(Motor_LensAlign_Y, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			checkTime = myTimer(true);
			MainStep = 422;
			break; 
		case 422:
			if (GetNegaSensor(Motor_LensAlign_Y))
			{
				StopAxis(Motor_LensAlign_Y);
			}
			if (GetHomeSensor(Motor_LensAlign_Y) || (GetNegaSensor(Motor_LensAlign_Y)) && IsStopAxis(Motor_LensAlign_Y))
			{
				checkTime = myTimer(true);
				MainStep = 423;
			}
		...
		case 423:
			...
			break;
		case 424:
			dAcc = sysData.fMotorAccTime[Motor_PCB_Xt];
			dVel = 3 * sysData.fMotorResol[Motor_PCB_Xt];
			AxmMoveSignalSearch(Motor_PCB_Xt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 430;
			break;
		case 430:
			dAcc = sysData.fMotorAccTime[Motor_PCB_Yt];
			dVel = 3 * sysData.fMotorResol[Motor_PCB_Yt];
			AxmMoveSignalSearch(Motor_PCB_Yt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 500;
			break;
		case 500://! Lens X축 (+) Limit 위치 이동
			...
			if((GetHomeSensor(Motor_Lens_Y) || GetNegaSensor(Motor_Lens_Y) )&& IsStopAxis(Motor_Lens_Y))
			{
				StopAxis(Motor_Lens_Y);
			...
			break;
		case 510:
			if (GetHomeSensor(Motor_Insp_X))
			{
				StopAxis(Motor_Insp_X);
			}
			if ((GetHomeSensor(Motor_Insp_X) || GetNegaSensor(Motor_Insp_X)) && IsStopAxis(Motor_Insp_X))
			{
				/*StopAxis(Motor_Insp_X);*/
				checkTime = myTimer(true);
				MainStep = 511;
			...
			break;
		case 511:
			dAcc = sysData.fMotorAccTime[Motor_Lens_Xt];
			dVel = 3 * sysData.fMotorResol[Motor_Lens_Xt];
			AxmMoveSignalSearch(Motor_Lens_Xt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 520;
			break;
		case 520:
			dAcc = sysData.fMotorAccTime[Motor_Lens_Yt];
			dVel = 3 * sysData.fMotorResol[Motor_Lens_Yt];
			AxmMoveSignalSearch(Motor_Lens_Yt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 530;
			break;
		case 530:
			dAcc = sysData.fMotorAccTime[Motor_LensAlign_Yt];
			dVel = 3 * sysData.fMotorResol[Motor_LensAlign_Yt];
			AxmMoveSignalSearch(Motor_LensAlign_Yt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 540;
			break;
		case 540:
			dAcc = sysData.fMotorAccTime[Motor_LensAlign_Xt];
			dVel = 3 * sysData.fMotorResol[Motor_LensAlign_Xt];
			AxmMoveSignalSearch(Motor_LensAlign_Xt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 541;
			break;
		case 541:
			dAcc = sysData.fMotorAccTime[Motor_LensLaser_Yt];
			dVel = 3 * sysData.fMotorResol[Motor_LensLaser_Yt];
			AxmMoveSignalSearch(Motor_LensLaser_Yt, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 542;
			break;
		case 542:
			dAcc = sysData.fMotorAccTime[Motor_Lens_TH];
			dVel = 3 * sysData.fMotorResol[Motor_Lens_TH];
			AxmMoveSignalSearch(Motor_Lens_TH, dVel, dAcc, PosEndLimit, SIGNAL_UP_EDGE, SLOWDOWN_STOP);
			Sleep(30);
			MainStep = 550;
			break;
		case 550://! PCB Y축 (-) Limit 위치 이동
			...
			if (GetPosiSensor(Motor_Lens_Xt) && IsStopAxis(Motor_Lens_Xt) && mLimitCheck[Motor_Lens_Xt] == false)
			{
				mLimitCheck[Motor_Lens_Xt] = true;
				dAcc = sysData.fMotorAccTime[Motor_Lens_Xt];
				dVel = -2 * sysData.fMotorResol[Motor_Lens_Xt];
				AxmMoveSignalSearch(Motor_Lens_Xt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_Lens_Yt) && IsStopAxis(Motor_Lens_Yt) && mLimitCheck[Motor_Lens_Yt] == false)
			{
				mLimitCheck[Motor_Lens_Yt] = true;
				dAcc = sysData.fMotorAccTime[Motor_Lens_Yt];
				dVel = -2 * sysData.fMotorResol[Motor_Lens_Yt];
				AxmMoveSignalSearch(Motor_Lens_Yt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_Lens_TH) && IsStopAxis(Motor_Lens_TH) && mLimitCheck[Motor_Lens_TH] == false)
			{
				mLimitCheck[Motor_Lens_TH] = true;
				dAcc = sysData.fMotorAccTime[Motor_Lens_TH];
				dVel = -2 * sysData.fMotorResol[Motor_Lens_TH];
				AxmMoveSignalSearch(Motor_Lens_TH, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_PCB_Xt) && IsStopAxis(Motor_PCB_Xt) && mLimitCheck[Motor_PCB_Xt] == false)
			{
				mLimitCheck[Motor_PCB_Xt] = true;
				dAcc = sysData.fMotorAccTime[Motor_PCB_Xt];
				dVel = -2 * sysData.fMotorResol[Motor_PCB_Xt];
				AxmMoveSignalSearch(Motor_PCB_Xt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_PCB_Yt) && IsStopAxis(Motor_PCB_Yt) && mLimitCheck[Motor_PCB_Yt] == false)
			{
				mLimitCheck[Motor_PCB_Yt] = true;
				dAcc = sysData.fMotorAccTime[Motor_PCB_Yt];
				dVel = -2 * sysData.fMotorResol[Motor_PCB_Yt];
				AxmMoveSignalSearch(Motor_PCB_Yt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_LensAlign_Yt) && IsStopAxis(Motor_LensAlign_Yt) && mLimitCheck[Motor_LensAlign_Yt] == false)
			{
				mLimitCheck[Motor_LensAlign_Yt] = true;
				dAcc = sysData.fMotorAccTime[Motor_LensAlign_Yt];
				dVel = -2 * sysData.fMotorResol[Motor_LensAlign_Yt];
				//AxmMoveSignalSearch(Motor_LensAlign_Yt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_LensAlign_Xt) && IsStopAxis(Motor_LensAlign_Xt) && mLimitCheck[Motor_LensAlign_Xt] == false)
			{
				mLimitCheck[Motor_LensAlign_Xt] = true;
				dAcc = sysData.fMotorAccTime[Motor_LensAlign_Xt];
				dVel = -2 * sysData.fMotorResol[Motor_LensAlign_Xt];
				//AxmMoveSignalSearch(Motor_LensAlign_Xt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_LensLaser_Yt) && IsStopAxis(Motor_LensLaser_Yt) && mLimitCheck[Motor_LensLaser_Yt] == false)
			{
				mLimitCheck[Motor_LensLaser_Yt] = true;
				dAcc = sysData.fMotorAccTime[Motor_LensLaser_Yt];
				dVel = -2 * sysData.fMotorResol[Motor_LensLaser_Yt];
				//AxmMoveSignalSearch(Motor_LensLaser_Yt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_LensLaser_Xt) && IsStopAxis(Motor_LensLaser_Xt) && mLimitCheck[Motor_LensLaser_Xt] == false)
			{
				mLimitCheck[Motor_LensLaser_Xt] = true;
				dAcc = sysData.fMotorAccTime[Motor_LensLaser_Xt];
				dVel = -2 * sysData.fMotorResol[Motor_LensLaser_Xt];
				//AxmMoveSignalSearch(Motor_LensLaser_Xt, dVel, dAcc, HomeSensor, SIGNAL_UP_EDGE, EMERGENCY_STOP);
				Sleep(100);
			}

			if (GetPosiSensor(Motor_Lens_X))
			{
				StopAxis(Motor_Lens_X);
		...
		case 560: 
			...
			if (mLimitCheck[Motor_Lens_Xt] == true && mLimitCheck[Motor_Lens_Yt] == true)
			{
				checkTime = myTimer(true);
				MainStep = 580;
			}
		...
		case 580:
			...
			if (mLimitCheck[Motor_PCB_Xt] == true && mLimitCheck[Motor_PCB_Yt] == true)
			{
				checkTime = myTimer(true);
				MainStep = 600;
			}
		...
		case 610:
			...
			break;
		case 620:
			if (GetHomeSensor(Motor_Lens_Xt) && GetHomeSensor(Motor_Lens_Yt))
			{
				iStep[Motor_Lens_Y] = 100;	// Lens Y 원점 시작
				MainStep = 621;
			}
			...
			{
				errMsg2(Task.AutoFlag, "[원점] Lens tx,ty축 (home)이동 실패");
				MainStep = -620;
			}
		...
		case 621:
			...
			break;
		case 622:
			if (GetPosiSensor(Motor_LensAlign_X))
			{
				StopAxis(Motor_LensAlign_X);
			}
			if ((GetHomeSensor(Motor_LensAlign_X) || GetPosiSensor(Motor_LensAlign_X)) && IsStopAxis(Motor_LensAlign_X))
			{
				StopAxis(Motor_LensAlign_X);
				MainStep = 800;
				break;
			...
			{
				errMsg2(Task.AutoFlag, "[원점] LensAlign_X 축 (+)이동 실패");
				/*errMsg2(Task.AutoFlag, sLangChange);*/
				MainStep = -621;
			}
			break;
		case 650:
			...
			if (GetHomeSensor(Motor_PCB_Yt) && IsStopAxis(Motor_PCB_Yt))
			{
				dAcc = sysData.fMotorAccTime[Motor_PCB_Yt];
				dVel = 5 * sysData.fMotorResol[Motor_PCB_Yt];
				AxmMoveVel(Motor_PCB_Yt, -dVel, dAcc, dAcc);
		...
		case 700: 
			...
			{
				sLangChange.LoadStringA(IDS_STRING335);
				errMsg2(Task.AutoFlag, "[원점] Pcb ty축 (-)이동 실패2");
				MainStep = -700;
			}
			break;
		case 800:
			...
			//{// 전체 Home 시작
				...
				iStep[Motor_Lens_TH] = 100;
				iStep[Motor_PCB_TH] = 100;	
				iStep[Motor_PCB_Xt] = 100;	
				iStep[Motor_PCB_Yt] = 100;	
				iStep[Motor_PCB_Z] = 100;
				iStep[Motor_LensAlign_X] = 100;
				iStep[Motor_LensAlign_Y] = 100;
				iStep[Motor_LensAlign_Z] = 100;
				//iStep[Motor_LensAlign_TH] = 100;
				iStep[Motor_LensAlign_Yt] = 100;
				iStep[Motor_LensAlign_Xt] = 100;
				iStep[Motor_LensLaser_Yt] = 100;
				iStep[Motor_LensLaser_Xt] = 100;
				MainStep = 900;
			//}
			break;
		...
		case 900:
			if((m_bOrgFlag[Motor_Lens_X] == true) && (m_bOrgFlag[Motor_Lens_Y] == true) && (m_bOrgFlag[Motor_Lens_Z] == true) && (m_bOrgFlag[Motor_Lens_Xt] == true) && 
				...
				(m_bOrgFlag[Motor_PCB_Xt] == true) && (m_bOrgFlag[Motor_PCB_Yt] == true) && (m_bOrgFlag[Motor_PCB_Z] == true) && (m_bOrgFlag[Motor_LensAlign_X] == true)
				&& (m_bOrgFlag[Motor_Insp_X] == true)
				&& (m_bOrgFlag[Motor_LensAlign_Y] == true) && (m_bOrgFlag[Motor_LensAlign_Z] == true)
				&& (m_bOrgFlag[Motor_LensAlign_Yt] == true)
				&& (m_bOrgFlag[Motor_LensAlign_Xt] == true) && (m_bOrgFlag[Motor_LensLaser_Yt] == true) && (m_bOrgFlag[Motor_LensLaser_Xt] == true))		// && (m_bOrgFlag[Motor_LensAlign_TH] == true) 
			{//! 전체 원점 검색 완료 Check
				m_bOrgFlag[Motor_LensAlign_TH] == true;
				MainStep = 1000;
			}
			break;
		}//main switch
...
bool CAxlMotorSet::HomeProcessAll_1st()
{
#if (__MACHINE_MODEL == MACHINE_1ST)
	CString sLog = "";
	CTimeChecker timeChk;
...
#else
	for(int i=0; i<Motor_Lens_TH; i++)
	{
		if (bThreadTaskReadyRun == true && i == Motor_Lens_Xt&& i == Motor_Lens_Yt&& i == Motor_Lens_TH)//if (bThreadTaskReadyRun == true && i==Motor_Lens_Yt && i==Motor_PCB_Yt&& i==Motor_PCB_Xt)	
			continue;
		if(m_bOrgFlag[i]==false)
			iHomeCheckFlag += 0x01 << i;
	}
...
bool CAxlMotorSet::Lens_LaserAlign_Motor_MoveZ(int index, bool evasionMode)
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	double eTime = 0.0;
	short axis[1];
	...
	if (!m_bOrgFlag[Motor_LensAlign_Z])
	{
	...
	if (index == Loading_Pos)
	{
		for (int i = Motor_LensAlign_Yt; i <= Motor_LensAlign_Xt; i++)
		{
			double dCheckPos = 0.0;

			if (i == Motor_LensAlign_Xt || i == Motor_LensAlign_Yt)		dCheckPos = GetCommandPos(i);
		...
		pos[0] = model.axis[Motor_LensAlign_Z].pos[Loading_Pos];
	}
	else if (index == Bonding_Pos)
	{
		for (int i = Motor_LensAlign_X; i <= Motor_LensAlign_Xt; i++)
		{
			if (i == Motor_LensAlign_Z)
			{
				continue;
			}
			double dCheckPos = 0.0;

			if (i == Motor_LensAlign_Xt || i == Motor_LensAlign_Yt)
				dCheckPos = GetCommandPos(i);
			else
				dCheckPos = GetEncoderPos(i);

			if (i == Motor_LensAlign_Z)		// z축 본딩 위치에 있을 경우 다시 본딩 위치로 이동 하라 했을 경우 error 발생.
			{
		...
		pos[0] = model.axis[Motor_LensAlign_Z].pos[Bonding_Pos];
	}
	else
	{

		pos[0] = model.axis[Motor_LensAlign_Z].pos[index];
	}
	axis[0] = Motor_LensAlign_Z;

	sLangChange.LoadStringA(IDS_STRING1501);	//Lens 부 %s 모터 이동 합니다.
	_stprintf_s(logMsg, sLangChange, MotorPosName[Motor_LensAlign_Z]);
	...
	while (1)
	{
		...
		case 100:								// 이동 완료 확인..
			if (IsStopAxis(Motor_LensAlign_Z))
				step = 1000;
			break;
		...
		if ((eTime - startTime)>MOTOR_MOVE_TIME)
		{
			if (step>100)
			{
				sLangChange.LoadStringA(IDS_STRING1502);	//Lens 부 [%s 모터] 이동 시간 초과
				_stprintf_s(logMsg, sLangChange, MotorPosName[Motor_LensAlign_Z]);
			}
			else
			{
				sLangChange.LoadStringA(IDS_STRING1503);	//Lens 부 %s 이동 비정상
				_stprintf_s(logMsg, sLangChange, MotorPosName[Motor_LensAlign_Z]);
			}
...
bool CAxlMotorSet::Lens_LaserAlign_Motor_MoveXY(int MoveType, int index, double offsetX, double offsetY)
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	double eTime = 0.0;
	short axis[6];
	...
	char logMsg[1000];
	CString logStr = _T("");
	for (int i = Motor_LensAlign_X; i<Motor_LensAlign_Z; i++)
	{
		if (!m_bOrgFlag[i])
		{
	...
	{
		sLangChange.LoadStringA(IDS_STRING1500);	//LensLaserAlign 부 모터 이동 위치가 비정상입니다.
	...
	axis[0] = Motor_LensAlign_X;
	axis[1] = Motor_LensAlign_Y;

	pos[0] = model.axis[Motor_LensAlign_X].pos[index];
	pos[1] = model.axis[Motor_LensAlign_Y].pos[index];
	...
	while (1)
	{
		...
		case 100:
			if (MoveType == 1)
			{
				if (IsStopAxis(Motor_LensAlign_Y))
				{
					goMotorPos(1, axis, pos);
					step = 101;
				}
			...
			{
				if (IsStopAxis(Motor_LensAlign_X))
				{
		...
		if (eTime - startTime>MOTOR_MOVE_TIME)
		{
			...
			{
				int motorNo = step - 100;
				_stprintf_s(logMsg, "LensLaserAlign 부 [%s 모터] %s 이동 시간 초과", MotorName[motorNo], MotorPosName[index]);
			}
			else
			{
				_stprintf_s(logMsg, "LensLaserAlign 부 %s 이동 비정상", MotorPosName[index]);
			}
			errMsg2(Task.AutoFlag, logMsg);
...
bool CAxlMotorSet::Lens_LaserAlign_Tilt_Move(int index, double offsetX, double offsetY)
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	char logMsg[1000];
	double eTime;
	...
	MotorPos[0] = GetCommandPos(Motor_LensAlign_Xt) - model.axis[Motor_LensAlign_Xt].pos[index];
	MotorPos[1] = GetCommandPos(Motor_LensAlign_Yt) - model.axis[Motor_LensAlign_Yt].pos[index];
	if (fabs(MotorPos[0]) > model.axis[Motor_LensAlign_Xt].m_dLimit_Err)
	{
		sLangChange.LoadStringA(IDS_STRING688);	//LENS TX 모터가 Limit를 벗어 났습니다. [Encoder %.03lf, Teaching %.03lf]
		_stprintf_s(logMsg, sLangChange, GetCommandPos(Motor_LensAlign_Xt), model.axis[Motor_LensAlign_Xt].pos[index]);
		errMsg2(Task.AutoFlag, logMsg);
		return false;
	}

	if (fabs(MotorPos[1]) > model.axis[Motor_LensAlign_Yt].m_dLimit_Err)
	{
		sLangChange.LoadStringA(IDS_STRING692);	//LENS TY 모터가 Limit를 벗어 났습니다. [Encoder %.03lf, Teaching %.03lf]
		_stprintf_s(logMsg, sLangChange, GetCommandPos(Motor_LensAlign_Yt), model.axis[Motor_LensAlign_Yt].pos[index]);
		errMsg2(Task.AutoFlag, logMsg);
		return false;
	}
	axis[0] = Motor_LensAlign_Xt;
	axis[1] = Motor_LensAlign_Yt;

	pos[0] = model.axis[Motor_LensAlign_Xt].pos[index];// + offsetX;
	pos[1] = model.axis[Motor_LensAlign_Yt].pos[index];// + offsetX;
	...
	while (1)
	{
		...
		case 100:								// 이동 완료 확인..
			if (IsStopAxis(Motor_LensAlign_Xt))			step = 101;
			break;

		case 101:
			if (IsStopAxis(Motor_LensAlign_Yt))		step = 1000;
			break;
		...
		if (eTime - startTime > MOTOR_MOVE_TIME)
		{
			StopAxis(Motor_LensAlign_Xt);
			StopAxis(Motor_LensAlign_Yt);
...
bool CAxlMotorSet::Lens_LaserAlign_Motor_Move(int index, double offsetX, double offsetY, double offsetTh)
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	double eTime;
	short axis[6];
	...
	double d_dist_X = model.axis[Motor_LensAlign_X].pos[index] - GetEncoderPos(Motor_LensAlign_X);
	double d_dist_Y = model.axis[Motor_LensAlign_Y].pos[index] - GetEncoderPos(Motor_LensAlign_Y);

	if (fabs(d_dist_X)>model.axis[Motor_LensAlign_X].m_dLimit_Err || fabs(d_dist_Y)>model.axis[Motor_LensAlign_Y].m_dLimit_Err)
	{
		axis[0] = Motor_LensAlign_Z;		// Z, Tilx X, Tilt Y 축 대기 위치 이동 후 X, Y, Θ, Tx, Ty 동작, 동작 완료 후 Z 동작..
		axis[1] = Motor_LensAlign_Xt;
		axis[2] = Motor_LensAlign_Yt;
		//axis[3] = Motor_LensAlign_TH;

		pos[0] = model.axis[Motor_LensAlign_Z].pos[index];
		pos[1] = model.axis[Motor_LensAlign_Xt].pos[index];
		pos[2] = model.axis[Motor_LensAlign_Yt].pos[index];
		//pos[3] = model.axis[Motor_LensAlign_TH].pos[index];
		...
		while (1)
		{
			...
			case 0:									// 5축 모터 대기 위치로 이동 명령..
				goMotorPos(3, axis, pos);			//goMotorPos(4, axis, pos);			// Z, Xt, Yt
				startTime = myTimer(true);
				step = 100;
				break;

			case 100:								// 이동 완료 확인..
				if (IsStopAxis(Motor_LensAlign_Z))		step = 101;
				break;

			case 101:
				if (IsStopAxis(Motor_LensAlign_Xt))		step = 102;
				break;

			case 102:
				if (IsStopAxis(Motor_LensAlign_Yt))	step = 1000; // 103;
				break;

			/*case 103:
				if (IsStopAxis(Motor_LensAlign_TH))	step = 1000;
				break;*/
			default:
				break;
			}
			...
			if (eTime - startTime>MOTOR_MOVE_TIME)
			{
				...
				{
					int motorNo = step - 100;
					sLangChange.LoadStringA(IDS_STRING1502);	//Lens 부 [%s 모터] %s 이동 시간 초과
	...
	for (int i = Motor_LensAlign_X; i <= Motor_LensAlign_Xt; i++)
	{
		...
		axis[i - Motor_LensAlign_X] = i;

		if (index == Bonding_Pos)								// Bonding_Pos 이동 시 Align 위치에서 인식한 x, y, Theta 보정값 적용..
		{
			if (fabs(offsetX)>model.axis[Motor_LensAlign_X].m_dLimit_Err || fabs(offsetY)>model.axis[Motor_LensAlign_Y].m_dLimit_Err)
			{
			...
			if (i == Motor_LensAlign_X)				pos[i - Motor_LensAlign_X] = model.axis[i].pos[index] + offsetX;
			else if (i == Motor_LensAlign_Y)		pos[i - Motor_LensAlign_X] = model.axis[i].pos[index] + offsetY;
			else if (i == Motor_LensAlign_Z)		pos[i - Motor_LensAlign_X] = model.axis[i].pos[index];
			else							pos[i - Motor_LensAlign_X] = model.axis[i].pos[index];
		}
		else
			pos[i - Motor_LensAlign_X] = model.axis[i].pos[index];
	}
	...
	/*else
	{
		double d_x_pos = abs(GetEncoderPos(Motor_LensAlign_X) - model.axis[Motor_LensAlign_X].pos[Bonding_Pos]);
		double d_y_pos = abs(GetEncoderPos(Motor_LensAlign_Y) - model.axis[Motor_LensAlign_Y].pos[Bonding_Pos]);
		double d_x2_pos = abs(GetEncoderPos(Motor_LensAlign_X) - model.axis[Motor_LensAlign_X].pos[Loading_Pos]);
		double d_y2_pos = abs(GetEncoderPos(Motor_LensAlign_Y) - model.axis[Motor_LensAlign_Y].pos[Loading_Pos]);*/

		// PCB x, y 모터가 본딩 위치 근처에 있으면..
	/*	if ((d_x_pos<model.axis[Motor_LensAlign_X].m_dLimit_Err) && (d_y_pos<model.axis[Motor_Lens_Y].m_dLimit_Err) || (d_x2_pos<model.axis[Motor_LensAlign_X].m_dLimit_Err) && (d_y2_pos<model.axis[Motor_Lens_Y].m_dLimit_Err))
		{
	...
	sLangChange.LoadStringA(IDS_STRING1501);//Lens 부 %s로 이동 합니다.	
	_stprintf_s(logMsg, sLangChange, MotorPosName[index]);
	...
	while (1)
	{
		...
		case 0:									// Lens 모터 대기 위치로 이동 명령..
			...
			{
				goMotorPos(0, axis, pos);
			}
			step = 100;
			break;

		case 100:								// 이동 완료 확인..
			if ((IsStopAxis(Motor_LensAlign_X) && GetInposition(Motor_LensAlign_X)) &&
				(IsStopAxis(Motor_LensAlign_Y) && GetInposition(Motor_LensAlign_Y)))
			{
				if (b_bonding_move_flag == 1)
				{
		...
		if (eTime - startTime > MOTOR_MOVE_TIME)
		{
			...
			{
				sLangChange.LoadStringA(IDS_STRING1500);	//Lens 부 %s 이동 비정상
				_stprintf_s(logMsg, sLangChange, MotorPosName[index]);
			}
			errMsg2(Task.AutoFlag, logMsg);
...
bool CAxlMotorSet::Lens_Motor_Move(int index, double offsetX, double offsetY, double offsetTh)
{
	...
	if (fabs(d_dist_X)>model.axis[Motor_Lens_X].m_dLimit_Err || fabs(d_dist_Y)>model.axis[Motor_Lens_Y].m_dLimit_Err)
	{
		...
		axis[1] = Motor_Lens_Xt;						
		axis[2] = Motor_Lens_Yt;						
		axis[3] = Motor_Lens_TH;

		pos[0] = model.axis[Motor_Lens_Z].pos[index];
		pos[1] = model.axis[Motor_Lens_Xt].pos[index];
		pos[2] = model.axis[Motor_Lens_Yt].pos[index];
		pos[3] = model.axis[Motor_Lens_TH].pos[index];
		...
		while(1)
		{
			...
			case 102:
				if ( IsStopAxis(Motor_Lens_Yt) )	step = 103;
				break;

			case 103:
				if (IsStopAxis(Motor_Lens_TH))	step = 1000;
				break;
			default:
				break;
			}
...
bool CAxlMotorSet::Lens_Motor_MoveXY(int MoveType, int index, double offsetX, double offsetY, bool bWait)
{
	double eTime = 0.0;
	short axis[6];
	double pos[6];
	...
	pos[0] = model.axis[Motor_Lens_X].pos[index] + offsetX;
	pos[1] = model.axis[Motor_Lens_Y].pos[index] + offsetY;

	step = 0;

	//int MoveType = 2;//0 = x,y동시 이동 , 1 = y축 부터  , 2 = x축 부터 이동
#if 0
	lensPosX = (float)model.axis[Motor_Lens_X].pos[Loading_Pos] - (float)GetEncoderPos(Motor_Lens_X);
	lensPosY = (float)model.axis[Motor_Lens_Y].pos[Loading_Pos] - (float)GetEncoderPos(Motor_Lens_Y);
	...
	while(1)
	{
		...
		case 100:
			...
			if (MoveType == 1)
			{
				lensPosY = model.axis[Motor_Lens_Y].pos[index] - GetEncoderPos(Motor_Lens_Y) + offsetY;
				if (IsStopAxis(Motor_Lens_Y) && fabs(lensPosY) < 0.05)
				{
					goMotorPos(1, axis, pos);
					step = 101;
				}
			...
			{
				lensPosX = model.axis[Motor_Lens_X].pos[index] - GetEncoderPos(Motor_Lens_X) + offsetX;
				if (IsStopAxis(Motor_Lens_X) && fabs(lensPosX) < 0.05)
				{
					goMotorPos(1, axis+1, pos+1);
					step = 101;
				}
...
bool CAxlMotorSet::Pcb_Motor_Move(int index, double offsetX, double offsetY, double offsetTh, bool bWait)
{
	...
	while (1)
	{
		...
		case 100:	// 이동 완료 확인..
			...
			if (bWait == false && b_bonding_move_flag == 0)
			{
				step = 1000;
				break;
			}
			if (IsStopAxis(Motor_PCB_X) && IsStopAxis(Motor_PCB_Y) && IsStopAxis(Motor_PCB_TH))
			{
				if (b_bonding_move_flag == 1)
				{
					goMotorPos(1, axis + 1, pos + 1);		// Y 이동..
		...
		case 101:
			...
			if (index == Bonding_Pos && sysData.m_iProductComp == 1)
			{
				if (Task.AutoFlag == 1)
				{
					...
					pcbPosY = fabs(GetEncoderPos(Motor_PCB_Y) - model.axis[Motor_PCB_Y].pos[index] + offsetY);
#elif (____AA_WAY == LENS_TILT_AA)
					pcbPosX = fabs(GetEncoderPos(Motor_PCB_X) - Task.dAAPcbMotor[0] + offsetX);	//PCB X access 저장된 값
					pcbPosY = fabs(GetEncoderPos(Motor_PCB_Y) - Task.dAAPcbMotor[1] + offsetY);	//PCB X access 저장된 값
#endif
				}
				else
				{
...
{	
	...
	dwReturn = AxmMotSetMoveUnitPerPulse(Motor_PCB_X, 1, 2000);// 10000);	//Unit/Pulse 변경 - X 
	dwReturn = AxmMotSetMoveUnitPerPulse(Motor_PCB_Y, 1, 2000);// 10000);	//					Y 
#endif
...
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	double posX = GetEncoderPos(Motor_Insp_X) - model.axis[Motor_Insp_X].pos[index];
	if (fabs(posX) < 0.1)
	{
		return true;
	}
...
bool CAxlMotorSet::Pcb_Motor_Move_Side_Laser(int PosIndex, double offsetX, double offsetY, double offsetTh)
{
	char logMsg[1000];
	double eTime = 0.0;
	short axis[2];
	...
	while (1)
	{
		...
		case 0:									// 3축 모터 대기 위치로 이동 명령..
			startTime = myTimer(true);
			goMotorPos(2, axis, pos);		//goMotorPos(3, axis, pos);
			step = 100;
			break;

		case 100:								// 이동 완료 확인..
			if (
				(IsStopAxis(Motor_PCB_X) && GetInposition(Motor_PCB_X)) &&
				(IsStopAxis(Motor_PCB_Y) && GetInposition(Motor_PCB_Y))
				)
			{
				step = 101;
...
bool CAxlMotorSet::Pcb_Move_Side_Tilt(double offsetTy, double offsetTh)
{
	char logMsg[1000];
	double eTime;
	short axis[3];
	...
	if (!m_bOrgFlag[Motor_PCB_Yt] || !m_bOrgFlag[Motor_PCB_TH])
	{
		sLangChange.LoadStringA(IDS_STRING1001);	//PCB 부 모터가 원점 복귀를 하지 않았습니다.
		_stprintf_s(logMsg, sLangChange);
		errMsg2(Task.AutoFlag, logMsg);
	...
	if (fabs(offsetTy)>model.axis[Motor_PCB_Yt].m_dLimit_Err)
	{
		sLangChange.LoadStringA(IDS_STRING997);	//PCB 부 X 축 보정값 허용 범위[± %.01f]를 초과 하였습니다
		_stprintf_s(logMsg, sLangChange, model.axis[Motor_PCB_X].m_dLimit_Err, offsetTy);
		errMsg2(Task.AutoFlag, logMsg);
	...
	{
		sLangChange.LoadStringA(IDS_STRING998);	//PCB 부 Y 축 보정값 허용 범위[± %.01f]를 초과 하였습니다.
		_stprintf_s(logMsg, sLangChange, model.axis[Motor_PCB_Y].m_dLimit_Err, offsetTh);
		errMsg2(Task.AutoFlag, logMsg);
	...
	pos[0] = GetEncoderPos(Motor_PCB_Yt) + offsetTy;
	pos[1] = GetEncoderPos(Motor_PCB_TH) + offsetTh;
	...
	while (1)
	{
		...
		case 100:								// 이동 완료 확인..
			if ((IsStopAxis(Motor_PCB_Yt) /*&& GetInposition(Motor_PCB_Yt)*/) &&
...
bool CAxlMotorSet::INSP_Motor_MoveX(int index, double offsetX, bool bWait)
{
#if (__MACHINE_MODEL == MACHINE______2ST)
	double eTime = 0.0;
	short axis[1];
	...
	if (!m_bOrgFlag[Motor_Insp_X])
	{
		sLangChange.LoadStringA(IDS_STRING1246);	//모터가 원점 복귀를 하지 않았습니다.
		_stprintf_s(logMsg, _T("%s ") + sLangChange, MotorName[Motor_Insp_X]);
		errMsg2(Task.AutoFlag, logMsg);
		return false;
	}
	...
	double lensCurrentPosY = 0.0;*/
	bool b_Z_Move_Flag = false;
	double d_dist_Y = model.axis[Motor_Insp_X].pos[index] - GetEncoderPos(Motor_Insp_X);


	axis[0] = Motor_Insp_X;


	pos[0] = model.axis[Motor_Insp_X].pos[index];
	...
	while (1)
	{
		...
		if (eTime - startTime>MOTOR_MOVE_TIME)
		{
			...
			{
				int motorNo = step - 100;
				_stprintf_s(logMsg, "Insp 부 [%s 모터] %s 이동 시간 초과", MotorName[motorNo], MotorPosName[index]);
			}
			else
			{
				_stprintf_s(logMsg, "Insp 부 %s 이동 비정상", MotorPosName[index]);
			}
			errMsg2(Task.AutoFlag, logMsg);
...
bool CAxlMotorSet::Pcb_Side_Align_Move(int index, double offsetTx)
{
	char logMsg[1000];
	double eTime;
	short axis[4];
	...
	CString strlog;
	if (!m_bOrgFlag[Motor_PCB_Xt])
	{
		sLangChange.LoadStringA(IDS_STRING1001);	//PCB 부 모터가 원점 복귀를 하지 않았습니다.
		_stprintf_s(logMsg, _T("%s ") + sLangChange, MotorName[Motor_PCB_Xt]);
		errMsg2(Task.AutoFlag, logMsg);
		return false;
	}
	axis[0] = Motor_PCB_Xt;
	pos[0] = model.axis[Motor_PCB_Xt].pos[index] + offsetTx;
	...
	{
		sLangChange.LoadStringA(IDS_STRING970);	//PCB X 모터가 Align 보정 Limit를 벗어 났습니다. [Encoder %.03lf, Teaching %.03lf]
		_stprintf_s(logMsg, sLangChange, GetCommandPos(Motor_PCB_Xt), model.axis[Motor_PCB_Xt].pos[Align_Pos]);
		errMsg2(Task.AutoFlag, logMsg);
		return false;
	}
	...
	while (1)
	{
		...
		case 100:								// 이동 완료 확인..
			//if (IsStopAxis(Motor_PCB_Xt) && GetInposition(Motor_PCB_Xt))			step = 1000;
			if (IsStopAxis(Motor_PCB_Xt))			step = 1000;
			break;
		default:
			break;
		}
		...
		if (eTime - startTime > MOTOR_MOVE_TIME)
		{
			...
			if (step > 100)
			{
				strlog.Format("PCB 부 [%s 모터] 이동 시간 초과", MotorName[Motor_PCB_Xt]);
			}
			else
			{
